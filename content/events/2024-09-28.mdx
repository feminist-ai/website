...
<Gallery images={[
  ["/images/initial_feminist_ai_lan_party_flyer.jpeg", "An announcement flyer announcing the initial Feminist AI LAN party with a cool 90s retro image and the text that says Come and network locally, experiment with LAN LLMs, build training datasets for feminist AIs, test fine-tuning with local GPUs, expose AI *isms via jailbreaks", "Announcement of initial event"],
  ["/images/clit_ipsum.png", "A very confusing looking response from ChatGPT that says the word tilc many times in a row, sometimes the words repeates also are muspi tilc", "Hacking ChatGPT for feminism"],
  ["/images/clit_ipsum_description.png", "A block of text from ChatGPT where it explains that Clit Ipsum is a humourous and playful version of Lorem Ipsum text.", "Thanks ChatGPT! Very helpful :)"]
]} />
